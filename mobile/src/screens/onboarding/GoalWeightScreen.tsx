/**
 * Goal Weight Screen (Step 4/12)
 * Q1 Onboarding - Enter goal weight
...
type GoalWeightScreenNavigationProp = StackNavigationProp<OnboardingStackParamList, 'GoalWeight'>;

interface GoalWeightScreenProps {
  navigation: GoalWeightScreenNavigationProp;
}

export default function GoalWeightScreen({ navigation }: GoalWeightScreenProps) {
  const { data, setGoalWeight } = useOnboardingStore();

  // Initialize with stored value or sensible default based on goal
  const getDefaultGoalWeight = () => {
    if (data.goalWeight) return data.goalWeight;
    if (!data.currentWeight) return 150; // fallback default

    // Suggest reasonable goal based on goal type
    if (data.goalType === 'lose_weight') {
      // Suggest 10% weight loss as starting point
      return Math.round(data.currentWeight * 0.9);
    } else if (data.goalType === 'gain_weight') {
      // Suggest 10% weight gain as starting point
      return Math.round(data.currentWeight * 1.1);
    }
    return data.currentWeight;
  };

  // Initialize with stored unit preference or default to lbs
  const [unit, setUnit] = useState<'lbs' | 'kg'>(data.weightUnit || 'lbs');
  const [weight, setWeight] = useState<number>(() => {
    const defaultWeightLbs = getDefaultGoalWeight();
    return unit === 'kg' ? Math.round(defaultWeightLbs * 0.453592) : defaultWeightLbs;
  });
  ...
  const handleContinue = () => {
    ...
    // If maintain weight, skip Goal Date and go to Personal Details
    if (data.goalType === 'maintain') {
      navigation.navigate('PersonalDetails');
    } else {
      navigation.navigate('GoalDate');
    }
  };
      ...
      <View style={styles.header}>
        ...
        <View style={styles.progressContainer}>
          <View style={styles.progressBar}>
            <View style={[styles.progressFill, { width: `${(4/12) * 100}%` }]} />
          </View>
        </View>
      </View>
